...
def get_errors(fileout,
    ...
    for output_filename in output_filenames:
        ...
        # Get phi errors
        phi = data_out["phi1"][:, 0, :]
        l2_err, linf_err = calculate_errors(phi, phi_bench)
...
def get_dfem_error():
    phi_bench = np.loadtxt("openmc/mesh_tally.txt")
    # phi_dfem = np.loadtxt("yak/phi_average_1000_500.txt")
    phi_dfem = np.loadtxt("yak/phi_angular_1000_10.txt")

    l2_err, linf_err = calculate_errors(phi_bench, phi_dfem)
