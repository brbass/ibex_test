...
for cell in [c_fuel, c_gap, c_clad, c_mod]:
    ...
    chi = mgxs.Chi(domain=cell, groups=groups, name="{}-chi".format(cell.name))
    sigma_f = mgxs.FissionXS(domain=cell, groups=groups, name="{}-sigma_f".format(cell.name))
    nu_sigma_f = mgxs.NuFissionMatrixXS(domain=cell, groups=groups, name="{}-nu_sigma_f".format(cell.name))
    kappa_sigma_f = mgxs.KappaFissionXS(domain=cell, groups=groups, name="{}-kappa_sigma_f".format(cell.name))
    nu = mgxs.MultiplicityMatrixXS(domain=cell, groups=groups, name="{}-nu".format(cell.name))
    
    for xs in [sigma_t, sigma_a, sigma_s, chi, sigma_f, nu_sigma_f, kappa_sigma_f, nu]:
